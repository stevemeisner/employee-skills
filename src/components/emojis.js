...
// xoxp-XXXXXXXXXX-XXXXXXXXXX-XXXXXXXXXX-XXXXXXXXXXXXXXXXXXXXXXXX
...
function Emojis() {
  // Replace with your actual token when running locally
  // DO NOT commit actual tokens to version control
  const apiToken = 'YOUR_SLACK_API_TOKEN_HERE'
  const slack = new Slack(apiToken)

  // Standalone flatten function instead of modifying Array prototype
  const flatten = (arr) =>
    arr.reduce((prev, cur) => {
    ...
    return new Promise((resolve, reject) => {
      slack.api('reactions.list', { user: id, count: 100 }, (err, response) => {
        ...
        reactions.forEach((r) => {
          totalCount += 1
          reactionsCounted[r] =
            reactionsCounted[r] != null ? reactionsCounted[r] + 1 : 1
        })
    ...
    return (
      getEmojis()
        ...
        .then((reactionsMap) => {
          ...
          reactionsMap.forEach((reactionsCounted) => {
            // each item is like emojiname -> use count
            for (const name in reactionsCounted) {
              if (ctx.allReactions[name] === undefined) {
                ctx.allReactions[name] = 0
              }
          ...
          Object.keys(ar)
            ...
            .forEach((name) => {
              ...
              if (ctx.ourEmojis.includes(name)) ourEmojisUsage[name] = ar[name]
                ...
                console.info(
                  name,
                  'not found in ourEmojisUsage, which means its system default'
                )
              }
            })
          // fill with 0 emojis that were not used at all
          ctx.ourEmojis.forEach(
            (name) => (ourEmojisUsage[name] = ourEmojisUsage[name] || 0)
          )
